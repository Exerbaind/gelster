...
for (let i = 0; i < menuBlocks.length; i++) {
  menuBlocks[i].addEventListener("click", function () {
    ...
    currentMaterialBlock = document.querySelector(
      ".fourth-block__content.fourth-block__content_hide"
    );
    activeMenuBlock = document.querySelector(
      ".th-content__item.content__item_act"
    );
...
const priceSmaragd = document.querySelectorAll(".price-now .smaragd"),
  ...
  priceTXevolutionUni = document.querySelectorAll(
    ".price-now .tx-evolution-uni"
  ),
  priceTXevolutionWood = document.querySelectorAll(
    ".price-now .tx-evolution-wood"
  ),
